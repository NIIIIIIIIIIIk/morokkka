...
export const AddEventForm: React.FC<AddEventFormProps> = ({ onAdd }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.addForm}>
      <h3 className={styles.formTitle}>NEW SCENE / SCHEDULE</h3>
      <div className={styles.formGroup}>
        <label>DATE:</label>
        <input type="text" placeholder="2026-06-01 or TBD" value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })} />
      </div>
      <div className={styles.formGroup}>
        <label>SCENE *</label>
        <input type="text" placeholder="Scene name" value={formData.scene} onChange={(e) => setFormData({ ...formData, scene: e.target.value })} required />
      </div>
      <div className={styles.formGroup}>
